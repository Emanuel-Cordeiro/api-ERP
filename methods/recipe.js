...
router.get('/', async (_, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar as receitas. Erro: ' + error });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao incluir/alterar a receita. Erro: ' + error });
  }
});
...
router.delete('/:id', async (req,res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao excluir a receita. Erro: ' + error });
  }
});
