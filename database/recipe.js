...
async function selectRecipes() {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    const ingredientSql = 'SELECT ingredient_id, quantity FROM recipe_ingredient WHERE recipe_id = $1';

    const ingredients = await databaseTransaction(ingredientSql, [recipe.recipe_id]);
...
async function selectRecipe(id) {
  let sql = 'SELECT recipe_id, description, cost FROM recipe WHERE recipe_id = $1';
  
  let result = await databaseTransaction(sql, [id]);
  
  let obj = result[0];

  sql = 'SELECT ingredient_id, quantity FROM recipe_ingredient WHERE recipe_id = $1';
...
async function insertRecipe(body) {
  ...
  const recipe_id = await databaseTransaction('SELECT MAX(recipe_id) FROM recipe');

  for (let i = 0; i < body[0].itens.length; i++) {
    sql = 'INSERT INTO recipe_ingredient (recipe_id, ingredient_id, quantity) VALUES ($1, $2, $3)';
